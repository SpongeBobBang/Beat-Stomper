...
class Main(Frame):
    def Get_Difficulty_Factor(self):
        return round(1+(self.__Plank_Count/50),2)
    ...
    def Load_Planks(self,*args):
        ...
        self.Fst_Plank = self.Load_Plank(self.Fst_X,self.Fst_Y,self.Plank_Width,self.Plank_Height,"#bdd7ec")
        self.Snd_Plank = self.Load_Plank(self.Snd_X,self.Snd_Y,self.Plank_Width,self.Plank_Height,"#bdd7ec")
        self.Trd_Plank = self.Load_Plank(self.Trd_X,self.Trd_Y,self.Plank_Width,self.Plank_Height,"#bdd7ec")       
    ...
    def Load_Block(self,*args):
        ...
        self.Block = self.Load_Plank(self.Block_X,self.Block_Y,0.6,self.Block_Height,"#59dd25")
    ...
    def Get_Plank_DeltaPerFrame_X(self,index,*args):
        if self.Plank_Loc_Y[index] < 960+25:
            DeltaPerFrame_X = self.Plank_Speed_X[index] * self.Get_Difficulty_Factor()
        else:
    ...
    def Get_Plank_DeltaPerFrame_Y(self,index,*args):
        if self.Plank_Loc_Y[index] < 960+25:
            DeltaPerFrame_Y = self.Plank_Speed_Y[index] * self.Get_Difficulty_Factor()
        else:
    ...
    def Move_Chcek(self,event):
        if event.char != 'p' and event.char != 'r':
            ...
            self.BG = self.Load_Plank(2.7,4.8,5.4,9.6,"#202020")
